...
export abstract class Utils {
    ...
    static handleError(
        ...
        form: FormGroup
    ): { serverErrors: ValidationError[]; generalError: string } {
        ...
        Object.keys(form.controls).forEach((fieldName) => {
            ...
            if (control) {
                control.valueChanges.subscribe(() => {
                    ...
                    // Изчисти generalError ако е подаден
                    if (generalError) {
                        generalError.set('');
                    }
                });
    ...
    static setupClearPasswordMismatchError(
        form: FormGroup,
    ...
    static getFieldError(
        ...
        if (field?.errors && field.touched) {
            ...
            if (field.errors["minlength"])
                return `Полето трябва да съдържа поне ${field.errors["minlength"].requiredLength} символа`;
    ...
    private static processValidationErrors(
        apiErrors: ValidationError[],
        form: FormGroup,
        genericMessage: string = "Възникна грешка"
    ): { serverErrors: ValidationError[]; generalError: string } {
    ...
    static getGeneralErrorMessage(error: ApiError): string {
        if (error?.error?.title || error?.error?.detail) {
            return (
                error.error.detail || error.error.title || "Грешка при регистрация"
            );
